...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmd,"/animlist",true)==0)
	{
        ...
        SendClientMessage(playerid,0xAFAFAFAA,"Anim: /crossarms /lay  /lay2 /lay3 /lay4 /hide /vomit /eat /wave /taichi");
        SendClientMessage(playerid,0xAFAFAFAA,"Anim: /deal /crack /smokem /smokef /groundsit /chat /f**ku");
        SendClientMessage(playerid,0xAFAFAFAA,"Camera: /cam1 /cam2 /cam3 /cam4 /cam5 /cam6 /cam7");
        SendClientMessage(playerid,0xAFAFAFAA,"Anim: /deal /pay /obosit");
        SendClientMessage(playerid,0xAFAFAFAA,"Mechanic: /fix1 /fix2");
        SendClientMessage(playerid,0xAFAFAFAA,"Cop: /cop1 /cop2");
        SendClientMessage(playerid,0xAFAFAFAA,"18+: /test555 /test551");
        
        //ApplyAnimation(playerid,"BLOWJOBZ","BJ_COUCH_LOOP_P",4.0,1,0,0,0,-1);
	}
 	...
 	if(strcmp(cmd, "/sit", true) == 0)
	{
 	...
 	if(strcmp(cmd, "/sit2", true) == 0)
	{
		ApplyAnimation(playerid,"BEACH", "ParkSit_M_loop", 4.0, 1, 0, 0, 0, 0); // Sit 2
        return 1;
    }
    // PEE
	...
	if(strcmp(cmd, "/beer", true) == 0)
	{
        ...
        PlayAudioStreamForPlayer(playerid,"http://k004.kiwi6.com/hotlink/3ud26vy7e0/liviu_guta_play_aj_marea_mea_placere.mp3");
    ...
    if(strcmp(cmd, "/drunk", true) == 0) {
		...
		PlayAudioStreamForPlayer(playerid,"http://k004.kiwi6.com/hotlink/u5554sfuld/Liviu_Pustiu_si_Play_AJ_vino_mama_sa_am_vezi.mp3");
    ...
    if(strcmp(cmd, "/lay3", true) == 0) {
		 LoopingAnim(playerid,"BEACH","ParkSit_W_loop",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/lay4", true) == 0) {
		 LoopingAnim(playerid,"BEACH","SitnWait_loop_W",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam1", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","camcrch_cmon",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam2", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","camcrch_idleloop",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam3", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","camcrch_stay",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam4", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","camstnd_cmon",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam5", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","piccrch_in",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam6", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","piccrch_take",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cam7", true) == 0) {
		 LoopingAnim(playerid,"CAMERA","picstnd_take",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/fix1", true) == 0) {
		 LoopingAnim(playerid,"CAR","Fixn_Car_Loop",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/fix2", true) == 0) {
		 LoopingAnim(playerid,"CAR","Fixn_Car_Out",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cop1", true) == 0) {
		 LoopingAnim(playerid,"COP_AMBIENT","Copbrowse_in",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/cop2", true) == 0) {
		 LoopingAnim(playerid,"COP_AMBIENT","Coplook_nod",4.0,1,0,0,0,0);
         return 1;
    }
    if(strcmp(cmd, "/deal", true) == 0) {
		 LoopingAnim(playerid,"DEALER","DEALER_DEAL",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/pay", true) == 0) {
		 LoopingAnim(playerid,"DEALER","shop_pay",4.0,1,0,0,0,0);
         return 1;
    }
    
    if(strcmp(cmd, "/obosit", true) == 0) {
    ...
    if(strcmp(cmd, "/mafia", true) == 0) {
    	PlayAudioStreamForPlayer(playerid,"http://k004.kiwi6.com/hotlink/93xtqq3i74/Liviu_Guta_A_sunat_o_babamp4.mp3");
